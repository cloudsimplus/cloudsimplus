...
public sealed interface StatisticalDistribution permits DiscreteDistribution, ContinuousDistribution {
     ...
     *
     * <p>Usually you shouldn't call this method but {@link #sample()} instead.</p>
     *
     ...
     * the returned value is manipulated to try reducing variance of generated random numbers.
     ...
     * (PRNG) used by implementing classes.
     *
     * <p>{@link Well19937c} is the PRNG also used by {@link RealDistribution}
     * implementations of the {@link org.apache.commons.math3}.
     * {@link ContinuousDistribution} implementations rely on classes in such a library to provide
     ...
     * providing a {@link RandomGenerator} when instantiating a {@link ContinuousDistribution}
     * allows the researcher to define any PRNG by calling the appropriate
     * {@link ContinuousDistribution} constructor.
     ...
     * (it's challenging to have both properties on the same PRNG).</p>
     *
     ...
     * to reduce variance of experiments using the generated numbers.
     *
     * <p>This technique doesn't work for all the cases. However,
     * in the cases it can be applied, in order for it to work, you have to
     * perform some actions. Consider an experiment that has to run "n" times.
     * The first half of these experiments has to use the seeds you want.
     * However, the second half of the experiments have to
     * set the {@code applyAntitheticVariates} attribute to {@code true}
     * and use the seeds of the first half of experiments.</p>
     *
     * Thus, the first half of experiments are run using PRNGs, returning
