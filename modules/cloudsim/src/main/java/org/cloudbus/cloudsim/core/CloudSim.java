 ...
 * file written in BRITE printFormatted is used to describe the network. Later,
 * nodes in such file are mapped to CloudSim entities. Delay calculated from the
 * BRITE model are added to the messages send through CloudSim. Messages using
 * the old model are converted to the appropriate methods with the correct
 * parameters.
 *
